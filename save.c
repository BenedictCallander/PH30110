...
{
    double *pressure = malloc(102*sizeof(double));
    double **Q = malloc(3*sizeof(double));
    for(i=0;i<=2;i++)
    {
        Q[i] = malloc(102*sizeof(double));
    }
    for(i=1;i<=100;i++)
    {
...
{
    FILE *ptr;
    ptr = fopen("rehash4.txt","w");
    //initialise state arrays
    double *energy =  malloc(102*sizeof(double));
    double *pressure =  malloc(102*sizeof(double));
    double *rho =  malloc(102*sizeof(double));
    double *mom =  malloc(102*sizeof(double));
    double *velocity =  malloc(102*sizeof(double)); 
    ...
    for(i=1;i<=100;i++)
    {
        ...
        energy[i] = Q[2][i];
        pressure[i] = ((gamma-1)*rho[i]*energy[i]); 
        fprintf(ptr,"%f,%f,%f,%f,%d\n", rho[i], velocity[i], energy[i], pressure[i],i);
    }
    return 0;
}
